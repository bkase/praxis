...
use crate::{
    doc::Doc, error::AethelCoreError, pack::load_packs_from_vault_impl,
...
pub(crate) fn validate_doc_impl(vault_root: &Path, doc: &Doc) -> Result<(), AethelCoreError> {
    ...
    let pack_name =
        doc.doc_type.split('.').next().ok_or_else(|| {
            AethelCoreError::Other(format!("Invalid type format: {}", doc.doc_type))
        })?;

    let pack = packs.iter().find(|p| p.name == pack_name).ok_or_else(|| {
        AethelCoreError::PackNotFound(pack_name.to_string(), vault_root.join("packs"))
    })?;

    let type_entry = pack.find_type(&doc.doc_type).ok_or_else(|| {
        AethelCoreError::PackTypeNotFound(doc.doc_type.clone(), pack_name.to_string())
    })?;

    // Get the compiled schema
    let schema = type_entry.compiled_schema.as_ref().ok_or_else(|| {
        AethelCoreError::SchemaCompilationError("Schema not compiled".to_string())
    })?;
