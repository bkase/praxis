...
use crate::{
    ...
    pack::{Pack, load_packs_from_vault_impl},
...
fn get_vault_lock(vault_root: &Path) -> Arc<Mutex<()>> {
    ...
    let canonical_path = vault_root.canonicalize().unwrap_or_else(|_| vault_root.to_path_buf());
    locks.entry(canonical_path)
...
fn find_doc_by_uuid(vault_root: &Path, uuid: &Uuid) -> Result<PathBuf, AethelCoreError> {
    ...
    let direct_path = docs_dir.join(format!("{}.md", uuid_str));
    ...
    Err(AethelCoreError::DocNotFound(
        *uuid,
        docs_dir,
    ))
}
...
fn generate_doc_path(vault_root: &Path, uuid: &Uuid) -> PathBuf {
    vault_root.join("docs").join(format!("{}.md", uuid))
}
...
fn write_file_atomic(path: &Path, content: &[u8]) -> Result<(), AethelCoreError> {
    ...
    fs::create_dir_all(dir)
        .map_err(|e| AethelCoreError::io_error(e, dir))?;
...
pub(crate) fn read_doc_impl(vault_root: &Path, uuid: &Uuid) -> Result<Doc, AethelCoreError> {
    ...
    let content = fs::read_to_string(&doc_path)
        .map_err(|e| AethelCoreError::io_error(e, &doc_path))?;
    
    Doc::from_markdown(&content)
}

/// Apply a patch to create or update a Doc (implementation)
pub(crate) fn apply_patch_impl(vault_root: &Path, patch: Patch) -> Result<WriteResult, AethelCoreError> {
    // Validate patch
    patch.validate()?;
    ...
    let (doc, is_new) = match (patch.uuid, patch.mode) {
        (None, PatchMode::Create) => {
            // Create new Doc
            let doc_type = patch.doc_type.as_ref()
                .ok_or_else(|| AethelCoreError::MissingRequiredField {
            ...
            let doc = Doc {
                ...
                tags: Vec::new(),
                frontmatter_extra: patch.frontmatter.clone()
...
fn find_pack_and_type<'a>(
    ...
    if parts.len() < 2 {
        return Err(AethelCoreError::Other(
            format!("Invalid type ID format: {}", type_id)
        ));
    }
    ...
    let type_entry = pack.find_type(type_id)
