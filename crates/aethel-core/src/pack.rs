    ...
    pub fn load(pack_path: &Path) -> Result<Self, AethelCoreError> {
        ...
        if !is_valid_pack_name(&manifest.name) {
            return Err(AethelCoreError::Other(
                format!("Invalid pack name '{}': must match ^[a-z0-9\\-]+$", manifest.name)
            ));
        }
        ...
        for type_entry in manifest.types {
            ...
            if !type_entry.id.starts_with(&expected_prefix) {
                return Err(AethelCoreError::Other(
                    format!("Type ID '{}' must start with '{}'", type_entry.id, expected_prefix)
                ));
            }
...
pub(crate) fn load_packs_from_vault_impl(vault_root: &Path) -> Result<Vec<Pack>, AethelCoreError> {
    ...
    for entry in fs::read_dir(&packs_dir)
        .map_err(|e| AethelCoreError::io_error(e, &packs_dir))?
    {
...
pub(crate) fn add_pack_impl(vault_root: &Path, source: &str) -> Result<Pack, AethelCoreError> {
    ...
    if dest_path.exists() {
        return Err(AethelCoreError::Other(
            format!("Pack '{}' already exists", dest_name)
        ));
    }
...
pub(crate) fn remove_pack_impl(vault_root: &Path, pack_name: &str) -> Result<bool, AethelCoreError> {
    let packs_dir = vault_root.join("packs");
            ...
            if let Some(dir_name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                if dir_name.starts_with(&format!("{}@", pack_name)) {
                    ...
                    fs::remove_dir_all(&path)
                        .map_err(|e| AethelCoreError::io_error(e, &path))?;
                    return Ok(true);
                }
            }
        }
...
fn copy_dir_recursive(src: &Path, dst: &Path) -> Result<(), AethelCoreError> {
    fs::create_dir_all(dst)
        .map_err(|e| AethelCoreError::io_error(e, dst))?;
    
    for entry in fs::read_dir(src)
        .map_err(|e| AethelCoreError::io_error(e, src))?
    {
        ...
        if src_path.is_dir() {
            ...
            fs::copy(&src_path, &dst_path)
                .map_err(|e| AethelCoreError::io_error(e, &src_path))?;
        }
    }
